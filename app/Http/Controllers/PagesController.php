...
class PagesController extends Controller
{
    ...
    {
        include_once (app_path() . '/functions/demo_map.php');
        $demo_marker = demo_map_listing();
        $page_title = 'Dashboard';
        $page_description = 'Some description for the page';
        $locations = DB::table( 'users' )->where( 'user_type', 'Service Provider' )->get( ['name','address1', 'long_lat'] );
        // dd($locations, $demo_marker);
        // $locations = User::select('address1', 'long_lat')->where( 'user_type', 'Service Provider' )->get();
        return view('pages.dashboard', compact('page_title', 'page_description', 'locations', 'demo_marker'));
    }
